...
- 当前 `manifest.json` 版本：**2.0.53**。当前插件名为 `Shelly素材库`。
...
- 看板展示：真实采集图按**自身宽高比例**排成瀑布流（CSS 多列），不裁成统一大小；图片上**不叠加任何标题/来源文案**（如 alt 文本、游戏名）。仅无图的演示占位卡才显示标题。
...
- 每套图（行级）操作包含：跳转商店、下载、标签、笔记、删除。跳转商店必须能打开该应用的详情页。
- 单图（缩略图）操作：悬停时右上角浮现两个小圆钮——上「收藏」、下「复制」；复制会把该单张图片写入剪贴板（失败回退复制地址）。这两个是悬停浮层，不占静态排版。
- 商店图标签为**多选**：点标签选项只切换选中态、不重绘弹层，可连选多个；点击空白处才关闭（与 Icon 标签一致）。不要改回“点一次标签就 render() 重绘”，否则每次只能选一个。
...
- 说明：`background.js` 中仍保留 `[Shelly]` 诊断日志（`console.info/warn`）。确认长期稳定后可降级或移除，届时记得升版本号并回写此处。

### 2.0.51 — 商店图单图复制按钮 + 收藏按钮微调（本会话，看板改动）

- 商店图每张缩略图右上角、收藏心形正下方新增复制按钮 `⧉`，悬停浮现，绝对定位浮层不占位；点击复用 `dashboard.js::copyAssetImage()` 复制该单张图片，并阻止触发放大查看。
- 商店图收藏按钮改小并上移（28→22px，top 8→5px）。仅影响商店图，不动灵感/Icon 卡片。
- 相关代码：`dashboard/dashboard.js` 商店行模板 `.store-thumb`（新增 `data-store-copy` 按钮）与 `storeRows` 点击委托；`dashboard/index.html` 的 `.store-copy` / `.store-favorite` 样式。

### 2.0.52 — 商店图标签可连选 + 灵感发现瀑布流（本会话，看板改动）

- 商店图标签：原来点一个标签就 `render()` 重绘导致弹层关闭、只能选一个。改为仅切换该标签选中态并 `persistWorkspace()`、不重绘，弹层保持打开可连选；点击空白处才关闭（沿用 §233 外部点击关闭逻辑）。与 Icon 标签行为一致。
- 灵感发现：解除 `.masonry.inspiration-grid .asset-art.has-image` 的固定 `aspect-ratio` 与 `object-fit:cover`，图片改 `width:100%/height:auto` 自适应，配合已有 CSS 多列布局形成真正的瀑布流。仅影响灵感发现。

### 2.0.53 — 灵感发现去除图片文案覆盖层（本会话，看板改动，当前版本）

- 用户反馈灵感发现采集图上叠了文案（alt 文本、来源游戏名等），不需要。隐藏灵感发现 `has-image` 卡片的 `.art-title` 文字覆盖层及其底部渐变 `:after`，呈现干净原图。
- 仅隐藏**显示**；素材内部仍保留 name/title（供下载文件名与搜索）。无图的演示占位卡仍保留标题；Icon/商店图不受影响。
